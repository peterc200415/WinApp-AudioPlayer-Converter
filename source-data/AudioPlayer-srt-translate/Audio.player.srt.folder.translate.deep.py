...
def update_subtitles():
    ...
    while pygame.mixer.music.get_busy() and current_subtitles and not is_closing:
        if is_paused:
            time.sleep(0.5)
            continue
        current_time = pygame.mixer.music.get_pos() / 1000.0  # 将毫秒转换为秒
...
def play_single_file(file_path):
    ...
    if os.path.exists(srt_file_path):
        ...
        def no_srt_message():
            ...
            translated_text_area.insert(tk.END, "未找到相關的 .srt 字幕文件。")
        if not is_closing:
            root.after(0, no_srt_message)
            ...
            for i in range(retry_count):
                try:
                    if os.path.exists(temp_wav_file.name):
                        ...
                        print(f"成功删除临时文件: {temp_wav_file.name}")
                        break
                except PermissionError:
                    print(f"删除临时文件时遇到问题，重试中... ({i + 1}/{retry_count})")
                    time.sleep(0.1)  # 等待一段时间再尝试
                except FileNotFoundError:
                    print(f"临时文件已不存在: {temp_wav_file.name}")
                    break
            else:
                print(f"无法删除临时文件: {temp_wav_file.name}")
